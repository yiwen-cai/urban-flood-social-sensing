...
- 模型供应商固定为 DeepSeek；批量分类默认使用 `deepseek-v4-flash`，经 20 条合成 fixture 冒烟测试后锁定具体版本
...
6. 明确 HumAID 官方许可标识与 QCRI 使用条款的差异；采取较严格口径：原始正文与未脱敏真实文本不得公开再分发；课堂投影 Slides 与现场演示可使用经 `redact.py` 脱敏后的正文，但不得将含真实脱敏正文的 deck/录屏上传至公开平台；
...
| 人道信息分类 | DeepSeek `deepseek-v4-flash` + Few-shot | 原生 9 类单标签；使用 JSON Output；不使用即将弃用的 `deepseek-chat` 别名 |
...
│   └── utils/
│       ├── io.py                   # JSONL 读写
│       ├── llm.py                  # DeepSeek API 调用、JSON 校验、重试与缓存
│       ├── cache.py                # 批次缓存与断点续跑
│       ├── redact.py               # 敏感信息脱敏
│       └── __init__.py
│
...
| DeepSeek API | OpenAI 兼容入口、V4 模型名与 JSON Output | [官方文档](https://api-docs.deepseek.com/) |
...
- [ ] 原始联系方式、姓名、账号和精确住址不出现在仓库、看板、Slides 或录屏（脱敏占位符 `[USER]`/`[NUMBER]`/`[EMAIL]` 除外）
- [ ] 公开仓库与可再分发材料不包含可搜索的原始帖子正文；课堂 Slides/现场演示可使用脱敏真实正文；可上传分享的 deck/录屏应使用聚合表或合成 fixture
- [ ] 项目不生成地图、时间趋势或逐帖时空结论
- [ ] 断网时看板、静态图、简报和录屏均可使用
...
- DeepSeek 已锁定为唯一模型供应商；默认 `deepseek-v4-flash`，仍需用 20 条合成 fixture 完成延迟、费用与 JSON 成功率冒烟测试
